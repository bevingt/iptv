...
class M3uParser:
    ...
    def __init__(self, useragent: str = None, timeout: int = 5):
        ...
        self._tvg_name_regex = re.compile(
            r"tvg-name=\"(.*?)\"", flags=re.IGNORECASE)
        self._tvg_id_regex = re.compile(
            r"tvg-id=\"(.*?)\"", flags=re.IGNORECASE)
        self._logo_regex = re.compile(
            r"tvg-logo=\"(.*?)\"", flags=re.IGNORECASE)
        self._category_regex = re.compile(
            r"group-title=\"(.*?)\"", flags=re.IGNORECASE)
        self._title_regex = re.compile(
            r"(?!.*=\",?.*\")[,](.*?)$", flags=re.IGNORECASE)
        self._country_regex = re.compile(
            r"tvg-country=\"(.*?)\"", flags=re.IGNORECASE)
        self._language_regex = re.compile(
            r"tvg-language=\"(.*?)\"", flags=re.IGNORECASE)
        self._tvg_url_regex = re.compile(
            r"tvg-url=\"(.*?)\"", flags=re.IGNORECASE)
    ...
    def parse_m3u(self, path: str, check_live: bool = True, enforce_schema: bool = True):
        ...
        if is_valid_url(path):
            ...
            try:
                with open(path, errors="ignore",encoding='utf8') as fp:
    ...
    def _parse_lines(self):
        ...
        coros = (self._parse_line(line_num) for line_num in range(
            num_lines) if "#EXTINF" in self._lines[line_num])
    ...
    def filter_by(
        ...
        def not_operator(x): return x if retrieve else not x
        try:
                    ...
                    lambda stream_info: any_or_all(
                        not_operator(
                            re.search(
                                ...
                                stream_info.get(key_0, {}).get(
                                    key_1, "") if nested_key else stream_info.get(key, ""),
                            )
                        )
                        for fltr in filters
                    ),
    ...
    def get_random_stream(self, random_shuffle: bool = True):
        ...
        if not len(self._streams_info):
            logging.error(
                "No streams information so could not get any random stream.")
            return
        if random_shuffle:
    ...
    def _get_m3u_content(self) -> str:
        ...
        for stream_info in self._streams_info:
            ...
            if stream_info.get("country") != None and stream_info["country"].get("code") != None:
                line += ' tvg-country="{}"'.format(
                    stream_info["country"]["code"])
            if stream_info.get("language") != None and stream_info["language"].get("name") != None:
                line += ' tvg-language="{}"'.format(
                    stream_info["language"]["name"])
    ...
    def to_file(self, filename: str, format: str = "json"):
        ...
        format = filename.split(
            ".")[-1] if len(filename.split(".")) > 1 else format
